...
function CheckBox({ value, onChange, className, ...props }: Props) {
  return (
    <div className="flex items-center gap-3 m-3" key={value}>
      <input
        className={cn(
          "relative float-left h-[1rem] w-[1rem] appearance-none rounded-sm border border-solid border-[#dcdcdc] outline-none ",
          "before:pointer-events-none before:absolute before:h-[0.875rem] before:w-[0.875rem] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:content-['']",
          ...
          "focus:transition-[border-color_0.2s] focus:before:scale-100 focus:before:opacity-[0.12] focus:after:absolute focus:after:z-[1] focus:after:block focus:after:h-[0.875rem] focus:after:w-[0.875rem] focus:after:rounded-[0.125rem] focus:after:content-[''] ",
      ...
      <label
        onChange={onChange}
        className="inline-block pl-[0.15rem] hover:cursor-pointer text-nowrap select-none"
        htmlFor={value}
      >
